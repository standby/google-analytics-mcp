    ...
    try:
        ...
        for tool in tools:
            if requires_approval(tool.name):
                ...
                assert prompt_name in ["approve_data_access", "approve_account_access"]
                sensitive_count += 1
        ...
        for prompt in prompts:
            assert len(prompt.arguments) > 0, f"Prompt {prompt.name} has no arguments"
            # All prompts should have 'operation' argument
            arg_names = [arg.name for arg in prompt.arguments]
            assert "operation" in arg_names, f"Prompt {prompt.name} missing 'operation' argument"
